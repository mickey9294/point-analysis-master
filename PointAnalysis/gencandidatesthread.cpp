...
GenCandidatesThread::GenCandidatesThread(int num_of_candidates, QObject *parent)
	: QThread(parent), m_num_of_candidates(num_of_candidates)
{
...
void GenCandidatesThread::generateCandidates()
{
	...
	for (part_cloud_it = parts_clouds.begin(); part_cloud_it != parts_clouds.end(); ++part_cloud_it)
	{
		...
		if (nvertices > 3)
		{
			...
			QVector<QList<int>> components_indices(num_of_components);    /* The indices of points which belong to each connected component */
				...
				* because it will cause an error to compute eigen vectors of the OBB which contains only 1 point */
				if (component_cand[j]->size() >= 3)
				{
					...
					for (int k = 0; k < cand_obbs.size(); k++)
					{
						...
						candidate.saveToFile(std::to_string(overall_cand_count++));
...
void GenCandidatesThread::loadCandidatesFromFiles()
{
	...
	{
		string cand_path = "../data/candidates/" + to_string(i) + ".txt";
