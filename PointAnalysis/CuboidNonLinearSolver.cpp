...
CuboidNonLinearSolver::CuboidNonLinearSolver(
	...
	, num_cuboid_corner_variables_(CuboidAttributes::k_num_attributes)
	, num_cuboid_axis_variables_(3 * 3)
	, num_reflection_symmetry_group_variables_(CuboidReflectionSymmetryGroup::num_axis_parameters())
	, num_rotation_symmetry_group_variables_(CuboidRotationSymmetryGroup::num_axis_parameters())
{
	...
	cuboid_axis_variable_start_index_ = cuboid_corner_variable_start_index_
		+ num_total_cuboid_corner_variables();  /* cuboid_coner_variable_start_index + 24 * 包围盒数 */
	reflection_symmetry_group_variable_start_index_ = cuboid_axis_variable_start_index_
		+ num_total_cuboid_axis_variables();  /* cuboid_axis_variable_start_index + 3 * 3 * 包围盒数 */
...
unsigned int CuboidNonLinearSolver::num_total_cuboid_corner_variables() const
{
	return num_cuboid_corner_variables_ * num_cuboids_;
}

unsigned int CuboidNonLinearSolver::num_total_cuboid_axis_variables() const
{
	return num_cuboid_axis_variables_ * num_cuboids_;
}

unsigned int CuboidNonLinearSolver::num_total_reflection_symmetry_group_variables() const
{
	return num_reflection_symmetry_groups_ * num_reflection_symmetry_group_variables_;
}

unsigned int CuboidNonLinearSolver::num_total_rotation_symmetry_group_variables() const
{
	return num_rotation_symmetry_groups_ * num_rotation_symmetry_group_variables_;
}

unsigned int CuboidNonLinearSolver::num_total_variables() const
{
...
NLPVectorExpression CuboidNonLinearSolver::create_vector_variable(
	const std::pair<Index, Index>& _index_size_pair)
{
	assert(_index_size_pair.second > 0);
	NLPVectorExpression variable(_index_size_pair.second);  /* _index_size_pair.second为维度，值为3 */
	for (unsigned int i = 0; i < _index_size_pair.second; ++i)
		variable[i] = NLPExpression(1.0, _index_size_pair.first + i);
	return variable;
}

std::pair<Index, Index> CuboidNonLinearSolver::get_cuboid_corner_variable_index_size(
	unsigned int _cuboid_index, unsigned int _corner_index) const
{
	const unsigned int dimension = 3;
	assert(_corner_index < OBB::k_num_corners);
...
std::pair<Index, Index> CuboidNonLinearSolver::get_cuboid_axis_variable_index_size(
...
std::pair<Index, Index> CuboidNonLinearSolver::get_reflection_symmetry_group_variable_n_index_size(
...
std::pair<Index, Index> CuboidNonLinearSolver::get_reflection_symmetry_group_variable_t_index_size(
	unsigned int _symmetry_group_index) const
{
	const unsigned int dimension = 3;
	assert(_symmetry_group_index < num_reflection_symmetry_groups_);
...
std::pair<Index, Index> CuboidNonLinearSolver::get_rotation_symmetry_group_variable_n_index_size(
	unsigned int _symmetry_group_index) const
{
	const unsigned int dimension = 3;
...
std::pair<Index, Index> CuboidNonLinearSolver::get_rotation_symmetry_group_variable_t_index_size(
	unsigned int _symmetry_group_index) const
{
	const unsigned int dimension = 3;
	assert(_symmetry_group_index < num_rotation_symmetry_groups_);
...
NLPVectorExpression CuboidNonLinearSolver::create_cuboid_corner_variable(
	unsigned int _cuboid_index, unsigned int _corner_index) const
{
...
NLPVectorExpression CuboidNonLinearSolver::create_cuboid_axis_variable(
	unsigned int _cuboid_index, unsigned int _axis_index) const
{
...
NLPVectorExpression CuboidNonLinearSolver::create_reflection_symmetry_group_variable_n(
	unsigned int _symmetry_group_index) const
{
...
NLPVectorExpression CuboidNonLinearSolver::create_reflection_symmetry_group_variable_t(
	unsigned int _symmetry_group_index) const
{
...
NLPVectorExpression CuboidNonLinearSolver::create_rotation_symmetry_group_variable_n(
	unsigned int _symmetry_group_index) const
{
...
NLPVectorExpression CuboidNonLinearSolver::create_rotation_symmetry_group_variable_t(
	unsigned int _symmetry_group_index) const
{
...
void CuboidNonLinearSolver::create_energy_functions(
	const Eigen::MatrixXd &_cuboid_quadratic_term,  /* mat_size行mat_size列，mat_size = 包围盒数 * 24 */
	const Eigen::VectorXd &_cuboid_linear_term,  /* mat_size维 */
	...
	std::vector<NLPFunction *> &_functions)
{
	...
	//
	NLPFunction *fuction_2 = create_reflection_symmetry_group_energy_function();  /* 构造反射对称能量函数 */
	assert(fuction_2);
	_functions.push_back(fuction_2);

	NLPFunction *fuction_3 = create_rotation_symmetry_group_energy_function();   /* 构造旋转对称能量函数 */
	assert(fuction_3);
	_functions.push_back(fuction_3);
	//
}

NLPFunction *CuboidNonLinearSolver::create_reflection_symmetry_group_energy_function()
{
	Eigen::MatrixXd quadratic_term = Eigen::MatrixXd::Zero(num_total_variables(), num_total_variables());
	Eigen::VectorXd linear_term = Eigen::VectorXd::Zero(num_total_variables());
	...
	delete_cuboid_sample_point_ann_trees(cuboid_ann_points, cuboid_ann_kd_tree);  /* 释放当前包围盒的搜索树 */

	NLPFunction *function = new NLPEigenQuadFunction(quadratic_term, linear_term, constant_term);
	return function;
}

void CuboidNonLinearSolver::create_reflection_symmetry_group_energy_function(
...
{
	const CuboidSymmetryGroup* symmetry_group = reflection_symmetry_groups_[_symmetry_group_index];
	assert(symmetry_group);

	std::list<CuboidSymmetryGroup::WeightedPointPair> sample_point_pairs;
	symmetry_group->get_symmetric_sample_point_pairs(cuboids_,
		...
		sample_point_pairs);  /* 获取模型上所有对称点对(包括反射对称和自对称) */
	...
	Real sum_weight = 0.0;
	/* 求所有对称点对的权值之和 */
	for (std::list<CuboidSymmetryGroup::WeightedPointPair>::iterator it = sample_point_pairs.begin();
	...
	for (std::list<CuboidSymmetryGroup::WeightedPointPair>::iterator it = sample_point_pairs.begin();
		it != sample_point_pairs.end(); ++it)  /* 对每一对对称点 */
	{
		Real weight = (*it).weight_ / sum_weight;  /* 归一化过后的权值 */

		Eigen::Vector3d sum_p, diff_p;  /* sum_p = p1 + p2; diff_p = p1 - p2 */
		for (int i = 0; i < 3; ++i)
		{
	...
	index_size_pair = get_reflection_symmetry_group_variable_n_index_size(_symmetry_group_index);  /* 获取当前这组对称的变量在所有特征中的起始位置 */
...
NLPFunction *CuboidNonLinearSolver::create_rotation_symmetry_group_energy_function()
{
	...
	for (unsigned int symmetry_group_index = 0; symmetry_group_index < num_rotation_symmetry_groups_;
		++symmetry_group_index)  /* 对于每个旋转对称关系 */
	{
		// Minimize \| t \|_2^2.
...
void CuboidNonLinearSolver::create_rotation_symmetry_group_energy_function(
	const unsigned int _symmetry_group_index,
	const std::vector<ANNpointArray>& _cuboid_ann_points,
	const std::vector<ANNkd_tree *>& _cuboid_ann_kd_tree,
	NLPExpression &_expression)
{
	const CuboidRotationSymmetryGroup* symmetry_group = rotation_symmetry_groups_[_symmetry_group_index];  /* 获取当前旋转对称关系 */
	assert(symmetry_group);
	...
	for (std::list<CuboidSymmetryGroup::WeightedPointPair>::iterator it = sample_point_pairs.begin();
		it != sample_point_pairs.end(); ++it)  /* 对于每一对对称点 */
	{
		Real weight = (*it).weight_ / sum_weight;  /* 归一化权值 */
		...
		Eigen::Vector3f symmetry_point_1 = symmetry_group->get_symmetric_point((*it).p1_, symmetry_order);  /* 获取p1的旋转对称点 */

		Eigen::Vector3d x_vec, y_vec;  /* 分别为代表着两个对称点的3维向量 */
		for (int i = 0; i < 3; ++i)
		{
			x_vec[i] = (*it).p1_[i];
			y_vec[i] = (*it).p2_[i];
...
void CuboidNonLinearSolver::add_constraints(NLPFormulation &_formulation)
{
...
void CuboidNonLinearSolver::add_cuboid_constraints(NLPFormulation &_formulation)
{
...
void CuboidNonLinearSolver::add_reflection_symmetry_group_constraints(NLPFormulation &_formulation)
{
	for (unsigned int symmetry_group_index = 0; symmetry_group_index < num_reflection_symmetry_groups_;
		++symmetry_group_index)
	{
...
void CuboidNonLinearSolver::add_rotation_symmetry_group_constraints(NLPFormulation &_formulation)
{
	for (unsigned int symmetry_group_index = 0; symmetry_group_index < num_rotation_symmetry_groups_;
		++symmetry_group_index)
	{
...
void CuboidNonLinearSolver::add_reflection_symmetry_group_constraints(
	const unsigned int _symmetry_group_index,
	NLPFormulation &_formulation)
{
	const CuboidReflectionSymmetryGroup *symmetry_group = reflection_symmetry_groups_[_symmetry_group_index];
	assert(symmetry_group);
...
void CuboidNonLinearSolver::add_reflection_constraints(
...
void CuboidNonLinearSolver::add_single_cuboid_reflection_constraints(
	const unsigned int _cuboid_index,
	const unsigned int _symmetry_group_index,
	...
	NLPFormulation &_formulation)
{
	...
	bool *is_corner_visited = new bool[OBB::k_num_corners];
	memset(is_corner_visited, false, OBB::k_num_corners * sizeof(bool));
...
void CuboidNonLinearSolver::add_pair_cuboid_reflection_constraints(
...
void CuboidNonLinearSolver::add_rotation_symmetry_group_constraints(
	const unsigned int _symmetry_group_index,
	NLPFormulation &_formulation)
{
	const CuboidRotationSymmetryGroup *symmetry_group = rotation_symmetry_groups_[_symmetry_group_index];
	assert(symmetry_group);
		...
		it != single_cuboid_indices.end(); ++it)
	{
		...
		cuboid_center_variable *= (1.0 / OBB::k_num_corners);
...
void CuboidNonLinearSolver::fix_cuboid(
	const unsigned int _cuboid_index,
	NLPFormulation &_formulation)
{
	const PAPart* cuboid = cuboids_[_cuboid_index];
	assert(cuboid);
	OBB * cuboid_obb = cuboid->getOBB();
	assert(cuboid_obb);
	Eigen::Matrix3f obb_axes = cuboid_obb->getAxes();
	...
	for (unsigned int corner_index = 0; corner_index < dimension; ++corner_index)
	{
		...
		Eigen::Vector3f axis = obb_axes.col(corner_index);
	...
	for (unsigned int corner_index = 0; corner_index < OBB::k_num_corners; ++corner_index)
	{
		...
		Eigen::Vector3f corner = obb_axes.col(corner_index);
...
bool CuboidNonLinearSolver::compute_initial_values(const Eigen::VectorXd &_input,
...
void CuboidNonLinearSolver::compute_cuboid_axis_values(Eigen::VectorXd &_values)
{
	assert(_values.rows() >= num_total_variables());

	for (unsigned int cuboid_index = 0; cuboid_index < num_cuboids_; ++cuboid_index)
	{
		const PAPart* cuboid = cuboids_[cuboid_index];
		assert(cuboid);
		OBB * cuboid_obb = cuboid->getOBB();
		assert(cuboid_obb);
		Eigen::Matrix3f obb_axes = cuboid_obb->getAxes();
		...
		{
			const Eigen::Vector3f &axis = obb_axes.col(axis_index);
...
void CuboidNonLinearSolver::compute_reflection_symmetry_group_values(Eigen::VectorXd &_values)
{
	assert(_values.rows() >= num_total_variables());
	...
	for (unsigned int symmetry_group_index = 0; symmetry_group_index < num_reflection_symmetry_groups_;
		++symmetry_group_index)
	{
		const CuboidReflectionSymmetryGroup* symmetry_group = reflection_symmetry_groups_[symmetry_group_index];
		assert(symmetry_group);
...
void CuboidNonLinearSolver::compute_rotation_symmetry_group_values(Eigen::VectorXd &_values)
{
	assert(_values.rows() >= num_total_variables());
	...
	for (unsigned int symmetry_group_index = 0; symmetry_group_index < num_rotation_symmetry_groups_;
		++symmetry_group_index)
	{
		const CuboidRotationSymmetryGroup* symmetry_group = rotation_symmetry_groups_[symmetry_group_index];
		assert(symmetry_group);
...
void CuboidNonLinearSolver::optimize(
	...
	const std::vector<unsigned int> *_fixed_cuboid_indices)
{
	...
	for (unsigned int symmetry_group_index = 0; symmetry_group_index < num_rotation_symmetry_groups_;
		++symmetry_group_index)
	{
		CuboidRotationSymmetryGroup* symmetry_group = rotation_symmetry_groups_[symmetry_group_index];
		assert(symmetry_group);
		symmetry_group->compute_rotation_angle(cuboids_);
	}

	std::vector<NLPFunction *> functions;  /* 目标函数 */
	create_energy_functions(_cuboid_quadratic_term, _cuboid_linear_term, _cuboid_constant_term, functions);  /* 构造目标函数 */
	NLPFormulation formulation(functions);
	add_constraints(formulation);   /* 添加约束 */
	...
	if (_init_values_vec)  /* _init_values_vec是所有包围盒8个顶点的坐标值 */
	{
...
void CuboidNonLinearSolver::update(const std::vector< Number >& _values)
{
...
void CuboidNonLinearSolver::update_cuboids(const std::vector< Number >& _values)
{
	const unsigned int dimension = 3;
	...
	for (unsigned int cuboid_index = 0; cuboid_index < num_cuboids_; ++cuboid_index)
	{
		...
		Eigen::Vector3f new_bbox_center = Eigen::Vector3f::Zero();
		std::array<Eigen::Vector3f, OBB::k_num_corners> new_bbox_corners;
		std::array<Eigen::Vector3f, dimension> new_bbox_axes;
		...
		new_bbox_center = new_bbox_center / OBB::k_num_corners;
		...
		for (unsigned int axis_index = 0; axis_index < dimension; ++axis_index)
		{
			...
			// Flip axes.
			Eigen::Vector3f minus_axis_direction = Eigen::Vector3f::Zero();
			Eigen::Vector3f plus_axis_direction = Eigen::Vector3f::Zero();
			...
			if (new_bbox_axes[axis_index].dot(obb_axes.col(axis_index)) < 0)
			{
				// Flip.
...
void CuboidNonLinearSolver::update_reflection_symmetry_groups(const std::vector< Number >& _values)
{
	const unsigned int dimension = 3;
...
void CuboidNonLinearSolver::update_rotation_symmetry_groups(const std::vector< Number >& _values)
{
	const unsigned int dimension = 3;
...
void CuboidNonLinearSolver::create_cuboid_sample_point_ann_trees(
	...
	std::vector<ANNkd_tree *>& _cuboid_ann_kd_tree) const
{
	...
	for (unsigned int cuboid_index = 0; cuboid_index < num_cuboids_; ++cuboid_index)  /* 对每个包围盒 */
	{
...
void CuboidNonLinearSolver::delete_cuboid_sample_point_ann_trees(
	std::vector<ANNpointArray>& _cuboid_ann_points,
	std::vector<ANNkd_tree *>& _cuboid_ann_kd_tree) const
{
